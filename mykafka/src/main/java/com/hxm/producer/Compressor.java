...
public class Compressor {
    ...
    public Compressor(ByteBuffer buffer,CompressionType type)  {
        ...
        // create the stream
        //ByteBufferOutputStream,封装的ByteBuffer，当写入数据超出容量时会自动扩容
        bufferStream = new ByteBufferOutputStream(buffer);
        //DataOutputStream,封装的ByteBufferOutputStream，提供压缩功能
        //根据压缩类型创建合适的压缩流
        //appendStream = new DataOutputStream(new GZIPOutputStream(bufferStream, COMPRESSION_DEFAULT_BUFFER_SIZE));
    ...
    public void close() {
        ...
        if (type != CompressionType.NONE) {
            ...
            buffer.putInt(pos - initPos - Record.LOG_OVERHEAD);
            ...
            int valueSize = pos - initPos - Record.LOG_OVERHEAD - Record.RECORD_OVERHEAD;
            buffer.putInt(initPos + Record.LOG_OVERHEAD + Record.KEY_OFFSET_V1, valueSize);
            // compute and fill the crc at the beginning of the message
            long crc = Record.computeChecksum(buffer,
                    initPos + Record.LOG_OVERHEAD + Record.MAGIC_OFFSET,
                    pos - initPos - Record.LOG_OVERHEAD - Record.MAGIC_OFFSET);
            Utils.writeUnsignedInt(buffer, initPos + Record.LOG_OVERHEAD + Record.CRC_OFFSET, crc);
            // reset the position
            buffer.position(pos);
    ...
    public void putLong(final long value) {
        try {
            ...
            throw new RuntimeException("I/O exception when writing to the append stream, closing", e);
        }
    }
